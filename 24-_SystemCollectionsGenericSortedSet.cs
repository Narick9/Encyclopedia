...
class SystemCollectionsGenericSortedSet_SystemConvert_DelegatesANDAnonymousFunctionANDLambda_Events
{
    static void Main()
    {
        Console.WriteLine("***** _ *****");

        SystemCollectionsGenericSortedSet_Silent();
        SystemConvert_Silent();
        DelegatesANDAnonymousFunctionANDLambda_Silent();
        Events_Silent();

        Console.ReadLine();
    }
    static void SystemCollectionsGenericSortedSet_Silent()
    {
        Console.WriteLine(">->->->->->->->->->->->->->->->->->->   SystemCollectionsGenericSortedSet_Silent()\n");


        // System.Collections.Generic.SortedSet - сортированное множество уникальных элементов (как в Python). Реализует (из
        //   System.Collections.Generic) ISet<T>, ICollection<T>, IEnumerable<T>, IReadOnlyCollection<T>, (из System.Collections)
        //   IEnumerable, ICollection
        /////////after reading///////////////////////////////////////////////////////////////////////
        //   , (из System.Runtime.Serialization) ISerializable, IDeserializationCallback
        /////////////////////////////////////////////////////////////////////////////////////////////
        // Стоит сказать, что положение элементов, как и равенство, определяется исходя из вывода метода System.IComparable<T>.CompareTo()
        //   элементов (или из System.Collections.Generic.IComparer<T>.Compare() объекта, что можно послать в одном из конструкторов)


        SortedSet<char> symbs = new SortedSet<char>("Humppakuski");  // System.Collections.Generic.SortedSet - множество, что
                                                                     //   сортируется при добавлении и удалении (зачем?) элементов
                                                                     // ctor() - всего 4-е конструктора (по умолчанию, принимающий
                                                                     //   System.Collections.Generic.IComparer<T>, принимающий
                                                                     //   System.Collections.Generic.IEnumerable<T> и смешанный).
                                                                     //   Можно было бы и использовать синтаксис инициализации коллекций
                                                                     // "Humppakuski" - здесь некоторые символы повторяются, но в нашей
                                                                     //   коллекции такого не будет
                                                                     //
        symbs.Add('r');                                              // symbs.Add() - коллекция сортируется при каждом добавлении (перегрузок у
        symbs.Add('i');                                              //   метода нет)
                                                                     // symbs.Add('i') - такой элемент у нас уже есть, и поэтому коллекция не
                                                                     //   изменится


        Console.WriteLine("<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<-<   SystemCollectionsGenericSortedSet_Silent()");
    }
    static void SystemConvert_Silent()
    {
        Console.WriteLine(">->->->->->->->->->->->->->->->->->->   SystemConvert_Silent()\n");


        //****maybe you should move it to next
