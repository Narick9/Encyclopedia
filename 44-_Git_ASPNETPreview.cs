...
class _Git_ASPNETPreview
{
    ...
    static void Git_Silent()
    {
        ...
        //       Есть такой раздел, git format-patch. Его суть в том, что она упаковывает commit в email (о ней есть инфо ниже). Первые строчки
        //           она использует в качестве загаловка к email-сообщению, а весь последующий текст помещает в тело
        //           
        ...
        //               --message=<msg>              - задать сообщение <msg>. С этим параметром ты пропускаешь часть с prompt'ом
        ...
        //           
        //       Раздел git branch имеет такие [options]:
        ...
        //           
        //       Раздел git switch имеет эти [options]:
        //               --create, -c <branch>       - перейти в ветку, что создатся прям сейчас (если <branch> уже есть - процесс остановится)
        //               --force-create, C <branch>  - то же, что и --create, но теперь, если <branch> уже имеется, процесс не остановится
        //           
        //           
        //   git restore
        //           
        ...
        //   git fetch
        //         
        //       Если тебе нужны лишь какие-то элементы из чужого репозитория, воспользуйся разделом git fetch. С ним ты легко можешь запросить
        //           копирование каких-то определённых объектов. Синопсис у него такой:
        //           
        //               git fetch [options] [<dir-with-repo> [<what-to-fetch>]]
        //           
        //           Вводи эти команды, находясь в проекте со своим репозиторием, и они будут привносить в этот твой репозиторий запрошенные
        //           вещи
        //           
        ...
        //           чужих репозиториев (или местных веток). Синопсис, конечно, не сложен:
        //           
        //               git pull [options] [<dir-with-repo>] [<branch>]
        //           
        //           Под <dirWithRepo> и <branch> подставляй репозиторий и его ветку, откуда хочешь стянуть модификации
        //         
        //       Допустим, мы хотим за'pull'ить себе поправки Alice. Мы сделаем это, перейдя в свой проект и применив git pull:
        //           
        //               git pull /home/alice/project/ master
        //           
        //           Эта команда фактически за'merge'ит её ветку master в нашу текущую ветку (сейчас у нас также margin). Конфликты файлов
        //           здесь также придётся решать
        //           
        //           
        //   git rebase
        //           
        ...
        //   git push
        //           
        //       Для разделяемых репозиториев
        //           
        //           
        //   git format-patch
        //           
        //       Подгатавливает email представление одного из твоих commit'ов (да, упоминание об этой утилитке есть выше). Синопсис таков:
        //           
